...
class ServerTunnelConnection: Connection {
	...
	func createTUNInterface() -> Int32 {
        ...
        let connectResult = withUnsafePointer(to: &socketAddressControl) {
            $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { sockaddrPtr in
                connect(utunSocket, sockaddrPtr, socklen_t(MemoryLayout<sockaddr_ctl>.size))
            }
        }
	...
	func setupVirtualInterface(address: String) -> Bool {
        ...
        guard let interfaceName = getTUNInterfaceName(utunSocket: utunSocket),
	...
	func readPackets() {
        ...
        var buffer: [UInt8] = Array(repeating: 0, count: Tunnel.packetSize)
		var protocolNumber: UInt32 = 0
        let protocolSize = MemoryLayout.size(ofValue: protocolNumber)
        let utunSocket = Int32(source.handle)

		repeat {
            // Keep the pointers passed to readv() valid for the duration of the call.
            let readCount: Int = withUnsafeMutablePointer(to: &protocolNumber) { protocolPointer in
                buffer.withUnsafeMutableBytes { bufferRaw -> Int in
                    var iovecList = [
                        iovec(iov_base: protocolPointer, iov_len: protocolSize),
                        iovec(iov_base: bufferRaw.baseAddress, iov_len: bufferRaw.count)
                    ]
                    return readv(utunSocket, &iovecList, Int32(iovecList.count))
                }
            }
            ...
            guard readCount > protocolSize else { break }
            ...
            packets.append(Data(bytes: buffer, count: readCount - protocolSize))
    ...
    override func sendPackets(_ packets: [Data], protocols: [NSNumber]) {
        ...
        for (index, packet) in packets.enumerated() {
            ...
            // Access the packet bytes only within the closure so the pointer stays valid for writev().
            let writeCount: Int = withUnsafeMutablePointer(to: &protocolNumber) { protocolPointer in
                packet.withUnsafeBytes { packetRaw -> Int in
                    var iovecList = [
                        iovec(iov_base: protocolPointer, iov_len: protocolSize),
                        iovec(iov_base: UnsafeMutableRawPointer(mutating: packetRaw.baseAddress), iov_len: packet.count)
                    ]
                    return writev(utunSocket, &iovecList, Int32(iovecList.count))
                }
            }
            ...
            else if writeCount < packet.count + protocolSize {
