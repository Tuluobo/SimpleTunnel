...
guard ServerTunnel.initializeWithConfigurationFile(path: configurationPath) else {
    print("Invalid config")
    exit(1)
}
guard let portNumber = Int32(portString) else {
	print("Invalid port: \(portString)")
	exit(1)
}
networkService = ServerTunnel.startListeningOnPort(port: portNumber)
