...
class UDPServerConnection: Connection {
    ...
    func getEndpointFromSocketAddress(socketAddressPointer: UnsafePointer<sockaddr>) -> (host: String, port: Int)? {
		...
		switch Int32(socketAddress.sa_family) {
			case AF_INET:
                var socketAddressInet = socketAddressPointer.withMemoryRebound(to: sockaddr_in.self, capacity: 1) { pointer in
                    UnsafePointer<sockaddr_in>(pointer).pointee
                }
			...
			case AF_INET6:
                var socketAddressInet6 = socketAddressPointer.withMemoryRebound(to: sockaddr_in6.self, capacity: 1) { pointer in
                    UnsafePointer<sockaddr_in6>(pointer).pointee
                }
        ...
        newResponseSource.setEventHandler {
            ...
            let bytesRead = withUnsafeMutablePointer(to: &socketAddress) {
                $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { sockaddrPtr in
                    response.withUnsafeMutableBytes { responseRaw in
                        recvfrom(UDPSocket, responseRaw.baseAddress, responseRaw.count, 0, sockaddrPtr, &socketAddressLength)
                    }
                }
			}
            ...
            let endpoint = withUnsafePointer(to: &socketAddress) {
                $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { sockaddrPtr in
                    self.getEndpointFromSocketAddress(socketAddressPointer: UnsafePointer(sockaddrPtr))
                }
            }
		...
		switch addressFamily {
                ...
                sent = withUnsafePointer(to: &serverAddress.sin) {
                    $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { pointer in
                        data.withUnsafeBytes {
                            sendto(UDPSocket, $0.baseAddress, data.count, 0, UnsafePointer(pointer), socklen_t(serverAddress.sin.sin_len))
                        }
                    }
				}
			...
			case AF_INET6:
                ...
                sent = withUnsafePointer(to: &serverAddress.sin6) {
                    $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { pointer in
                        data.withUnsafeBytes {
                            sendto(UDPSocket, $0.baseAddress, data.count, 0, UnsafePointer(pointer), socklen_t(serverAddress.sin6.sin6_len))
                        }
                    }
				}
